...
Paillier::Paillier(const long keyLength = 512){
    ...
    u = NTL::InvMod(L_function(lambda_power), modulus);						// u = L((generator^lambda) mod n ^ 2) ) ^ -1 mod modulus
}

NTL::ZZ Paillier::add(NTL::ZZ& x, NTL::ZZ& y){
...
NTL::ZZ Paillier::mul(NTL::ZZ& x, NTL::ZZ& y){
...
NTL::ZZ Paillier::encrypt(const NTL::ZZ& message) {
    ...
    NTL::ZZ ciphertext = NTL::PowerMod(generator, message, modulus * modulus) * NTL::PowerMod(random, modulus, modulus * modulus) % (modulus * modulus);
    return ciphertext;
}
